 ...
 *   (0,1) (1,2)     => 0
 */
function vectorModulus(a, b) {
    return Math.sqrt(a ** 2 + b ** 2);
}
function getAngleBetweenVectors(x1, y1, x2, y2) {
    const dotproduct = (x1 * x2) + (y1 * y2);
    const mod1 = vectorModulus(x1, y1);
    const mod2 = vectorModulus(x2, y2);
 ...
 */
function getParallelipidedDiagonal(...args) {
    const squaredArgs = args.map((item) => (item ** 2));
    return Math.sqrt(squaredArgs.reduce((a, b) => (a + b), 0))
